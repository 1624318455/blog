...
利用 pre-commit、post-merge 等钩子自动化工作流。`,
  ...
  {
    id: 8,
    title: '打造个人开发知识管理系统：定时备份与飞书通知实战',
    excerpt: '本文记录了如何构建一个完整的开发配置与知识管理自动化系统，包括每日自动备份全局配置和项目文件到 GitHub、任务执行状态实时推送到飞书群、以及技术沉淀的自动发布流程。',
    content: `# 打造个人开发知识管理系统：定时备份与飞书通知实战

> 本文记录了如何构建一个完整的开发配置与知识管理自动化系统，包括每日自动备份、飞书实时通知、以及技术沉淀的自动发布流程。

## 背景

作为一名开发者，我们日常使用 OpenCode 进行 AI 辅助编程，积累了大量的配置信息（AGENTS.md）和项目经验（memory.md）。这些内容如果丢失，将是巨大的损失。

因此，我构建了一套自动化系统来实现：
1. 每日自动备份全局配置和项目文件到 GitHub
2. 任务执行状态实时推送到飞书群
3. 将项目中的经验总结自动发布到技术博客
...
│  ┌─────────────────┐ ┌──────────────────┐ ┌─────────────┐ │
│  │ backup-global   │ │ backup-projects   │ │ publish-    │ │
│  │ .ps1           │ │ .ps1             │ │ summary.ps1 │ │
│  └─────────────────┘ └──────────────────┘ └─────────────┘ │
└─────────────────────────────────────────────────────────────┘
                               │
           ...
           ▼                   ▼                   ▼
┌──────────────────┐  ┌──────────────┐  ┌─────────────────┐
│  GitHub 仓库      │  │ 飞书机器人   │  │ Blog 草稿       │
│  opencode-backup │  │ Detroit     │  │ daily-summary   │
└────────────────────┘  └──────────────┘  └─────────────────┘
\`\`\`
...
function Send-FeishuMessage {
    ...
    npx -y @larksuite/cli im +messages-send --chat-id \$CHAT_ID --text \$text
}

# 开始时发送通知
Send-FeishuMessage -text "📦 开始执行全局配置备份任务..."

# 执行备份逻辑
# ... git clone, copy files, commit, push ...

# 完成时发送通知
\$successMsg = "✅ 全局配置备份完成！"
Send-FeishuMessage -text \$successMsg
\`\`\`

### 2. Windows 定时任务

使用 \`schtasks\` 创建每日定时任务：

\`\`\`powershell
# 创建全局备份任务
schtasks /Create /F /TN "OpenCode\\\\opencode-daily-backup-global" \\
    /TR "powershell.exe -ExecutionPolicy Bypass -File D:\\\\Work\\\\ForAI\\\\scripts\\\\backup-global.ps1" \\
    /SC DAILY /ST 18:00
\`\`\`

### 3. 飞书消息效果

任务执行时会在"Detroit家人群"收到如下消息：
...
飞书 CLI 在有代理的环境下可能出现问题，解决方法：

\`\`\`powershell
$env:LARK_CLI_NO_PROXY = "1"  # 禁用代理
\`\`\`

### 2. opencode-scheduler 的局限性

- 在 Windows 上使用 \`schedule_job\` 工具创建的任务会失败
- 解决方案：直接使用 \`schtasks\` 创建 Windows 原生任务

### 3. 备份仓库策略

- 创建私有仓库避免敏感信息泄露
- 使用 \`gh auth\` 登录后直接推送，无需每次输入凭据

## 技术栈

| 组件 | 用途 |
|------|------|
| Windows Task Scheduler | 定时触发任务 |
| PowerShell | 脚本逻辑执行 |
| Git + GitHub | 配置版本化管理 |
| 飞书 CLI | 消息通知机器人 |
| OpenCode | AI 编程辅助 |
...
这套系统实现了：
- **零丢失**：每日自动备份，配置和经验永不丢失
- **实时感知**：任务状态实时推送，不错过任何执行结果
- **知识沉淀**：经验总结自动提取，形成技术文章

希望这个方案能给有类似需求的开发者一些参考。如果你也有更好的实现方式，欢迎交流探讨！

---

*本文由 AI 辅助编写，自动化系统驱动的知识管理系统*`,
    author: 'memeflyfly',
    date: '2026-04-05',
    tags: ['automation', 'feishu', 'github', 'backup', 'windows'],
    views: 7,
  },
];